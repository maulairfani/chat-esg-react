...
export function setupAuth(app: Express) {
  ...
  const sessionSettings: session.SessionOptions = {
    secret: process.env.REPL_ID || "chat-esg-secret",
    resave: false,
  ...
  app.post("/api/register", async (req, res, next) => {
    try {
      ...
      if (!result.success) {
        return res
          .status(400)
          .send("Input tidak valid: " + result.error.issues.map(i => i.message).join(", "));
      }
  ...
  app.post("/api/login", (req, res, next) => {
    ...
    const cb = (err: any, user: Express.User, info: IVerifyOptions) => {
      ...
      if (!user) {
        return res.status(400).send(info.message ?? "Login gagal");
      }
